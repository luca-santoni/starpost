...
```powershell
& "$env:LOCALAPPDATA\Programs\Inno Setup 6\ISCC.exe" /DMyAppVersion=1.4.0 packaging\starpost.iss
```
...
`StarPost-<version>-Setup.exe`** (e.g. `StarPost-1.4.0-Setup.exe`); this is
...
The version defaults to `1.4.0` inside the script but **should be passed on the
